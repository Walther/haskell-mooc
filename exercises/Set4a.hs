...
longest xs = maximumBy (\x y -> compare (length x) (length y)) $ reverse $ sort xs
...
incrementKey :: (Ix k, Num v) => k -> [(k,v)] -> [(k,v)]
incrementKey a = map (\x -> (fst x, maybeInc a x))

maybeInc a x = if fst x == a then snd x+1 else snd x
