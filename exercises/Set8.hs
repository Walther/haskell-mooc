...
combine func (Picture f1) (Picture f2) = Picture (\coord -> func (f1 coord) (f2 coord))
...
union a b = Shape (\(Coord x y) -> contains a x y || contains b x y)

cut :: Shape -> Shape -> Shape
cut a b = Shape (\(Coord x y) -> contains a x y && not (contains b x y))
