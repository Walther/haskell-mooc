...
member x (Set n) = x `elem` n

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add x (Set []) = Set [x]
add x (Set n) = if member x (Set n) then Set n else Set (sort (x : n))
...
data State = Start | Eggy | Floury | Sugary | FlourySugary | Mixed | Error | Finished
  deriving (Eq, Show)
...
step state event = case state of
  ...
  Eggy -> case event of
    ...
    AddSugar -> Sugary
    _ -> Error
  Floury -> case event of
    AddSugar -> FlourySugary
    _ -> Error
  Sugary -> case event of
    AddFlour -> FlourySugary
    _ -> Error
  FlourySugary -> case event of
    Mix -> Mixed
    _ -> Error
  Mixed -> case event of
    Bake -> Finished
