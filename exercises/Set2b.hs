...
myGcd a 0 = a
myGcd 0 b = b
myGcd a b = myGcd smaller (larger - smaller)
  where
    smaller = min a b
    larger = max a b
...
countdown x = "Ready! " ++ down x

down 0 = "Liftoff!"
down x = show x ++ "... " ++ down (x -1)
...
biggestPrimeAtMost n = last $ takeWhile (<= n) $ filter isPrime [2 ..]
